...
pub.get('/auth/nextcloud', function (_req, res) {
...
pub.get('/auth/nextcloud/callback', function (req, res) {
    ...
    */
    ...
    nextcloudAuth.code.getToken(req.originalUrl, stateOpt).then(function (user: User) {
        queryUser(user.data.user_id, function (err: any, result: any) {
            if (err) {
                console.error(err.stack);
                res.statusCode = 500;
                return res.end();
            } else if (result && result.length) {
                deleteUser(user.data.user_id, function (err: any) {
                    if (err) {
                        console.error(err.stack);
                        res.statusCode = 500;
                        return res.end();
                    }
                    insertUser(user, function(err: any) {
                        if (err) {
                            console.error(err);
